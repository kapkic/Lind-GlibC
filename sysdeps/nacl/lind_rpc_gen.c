...
int lind_bind_rpc (int sockfd, socklen_t addrlen,
...
int lind_connect_rpc (int sockfd, socklen_t addrlen,
		      const struct sockaddr *src_addr);
int lind_listen_rpc (int sockfd, int backlog);
int lind_sendto_rpc (int sockfd, size_t len, int flags, socklen_t addrlen,
		     const struct sockaddr_in *dest_addr, const void *buf);
int lind_accept_rpc (int sockfd, socklen_t addrlen);
int lind_getpeername_rpc (int sockfd, socklen_t addrlen_in,
			  __SOCKADDR_ARG addr, socklen_t * addrlen_out);
int lind_setsockopt_rpc (int sockfd, int level, int optname, socklen_t optlen,
			 const void *optval);
int lind_getsockopt_rpc (int sockfd, int level, int optname, socklen_t optlen,
			 void *optval);
int lind_shutdown_rpc (int sockfd, int how);
int lind_select_rpc (int nfds, fd_set * readfds, fd_set * writefds,
		     fd_set * exceptfds, struct timeval *timeout,
		     struct select_results *result);
int lind_getifaddrs_rpc (int ifaddrs_buf_siz, void *ifaddrs);
int lind_recvfrom_rpc (int sockfd, size_t len, int flags, socklen_t addrlen,
		       socklen_t * addrlen_out, void *buf,
		       struct sockaddr *src_addr);
int lind_poll_rpc (int nfds, int timeout, struct pollfd *fds_in,
...
int
lind_access_rpc (int version, const char *file)
{
...
// check if file is NULL
  ...
  request.format =
    combine (4, "", "<i", strlenfile_file_len_str, file_fmt_str);
  ...
  nacl_rpc_syscall_proxy (&request, reply_buffer, 1, file_ptr,
			  file_data_size);
...
int
lind_unlink_rpc (const char *name)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
// check if name is NULL
  ...
  request.message.body = &args;
  nacl_rpc_syscall_proxy (&request, reply_buffer, 1, name_ptr,
			  name_data_size);
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
    {
...
int
lind_link_rpc (const char *from, const char *to)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
// check if to is NULL
  ...
  request.format =
    combine (5, "", strlenfrom_from_len_str, from_fmt_str,
	     strlento_to_len_str, to_fmt_str);
  ...
  request.message.body = &args;
  nacl_rpc_syscall_proxy (&request, reply_buffer, 2, from_ptr, from_data_size,
			  to_ptr, to_data_size);
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
    {
...
int
lind_chdir_rpc (const char *name)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
int
lind_mkdir_rpc (int mode, const char *path)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
// check if path is NULL
  ...
  request.format =
    combine (4, "", "<i", strlenpath_path_len_str, path_fmt_str);
  ...
  request.message.body = &args;
  nacl_rpc_syscall_proxy (&request, reply_buffer, 1, path_ptr,
			  path_data_size);
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
    {
...
int
lind_rmdir_rpc (const char *path)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
int
lind_xstat_rpc (int version, const char *path, struct stat *buf)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
  ...
  else
    {
      ...
      if (ptr != NULL)
	{
	  memcpy (ptr, ((&(reply_buffer->contents)) + 0),
		  sizeof (struct stat));
	}
    }
  free (reply_buffer);
...
int
lind_open_rpc (int fd, int mode, const char *path)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
// check if path is NULL
  ...
  assert (strcmp (path_fmt_str, "ZZZ") != 0);
  request.format =
    combine (5, "", "<i", "<i", strlenpath_path_len_str, path_fmt_str);
...
int
lind_read_rpc (int fd, int size, void *buf)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
  ...
  else
    {
      ...
      if (ptr != NULL)
	{
	  memcpy (ptr, ((&(reply_buffer->contents)) + 0), size);
	}
    }
  free (reply_buffer);
  return return_code;
...
int
lind_write_rpc (int desc, size_t count, const void *buf)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
// check if buf is NULL
  ...
  request.format =
    combine (5, "", "<i", "<I", count_buf_len_str, buf_fmt_str);
...
int
lind_lseek_rpc (off_t offset, int fd, int whence, off_t * ret)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
  ...
  else
    {
      ...
      if (ptr != NULL)
	{
	  memcpy (ptr, ((&(reply_buffer->contents)) + 0), sizeof (off_t));
	}
    }
  free (reply_buffer);
  return return_code;
...
int
lind_fxstat_rpc (int fd, int version, struct stat *buf)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
int
lind_fstatfs_rpc (int fd, struct statfs *buf)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
int
lind_statfs_rpc (const char *path, struct statfs *buf)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
  ...
  else
    {
      ...
      if (ptr != NULL)
	{
	  memcpy (ptr, ((&(reply_buffer->contents)) + 0), sizeof (pid_t));
	}
    }
  free (reply_buffer);
  return return_code;
...
int
lind_dup2_rpc (int oldfd, int newfd)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
int
lind_getdents_rpc (int fd, size_t nbytes, char *buf)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
  ...
  else
    {
      ...
      if (ptr != NULL)
	{
	  memcpy (ptr, ((&(reply_buffer->contents)) + 0), nbytes);
	}
    }
  free (reply_buffer);
  return return_code;
...
int
lind_fcntl_get_rpc (int fd, int cmd)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
int
lind_fcntl_set_rpc (int fd, int cmd, long set_op)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
int
lind_socket_rpc (int domain, int type, int protocol)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
int
lind_bind_rpc (int sockfd, socklen_t addrlen, const struct sockaddr *addr)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
// check if addr is NULL
  ...
  request.format =
    combine (5, "", "<i", "<i", addrlen_addr_len_str, addr_fmt_str);
  ...
  request.message.body = &args;
  nacl_rpc_syscall_proxy (&request, reply_buffer, 1, addr_ptr,
			  addr_data_size);
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
    {
...
int
lind_send_rpc (int sockfd, size_t len, int flags, const void *buf)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
// check if buf is NULL
  ...
  assert (strcmp (buf_fmt_str, "ZZZ") != 0);
  request.format =
    combine (6, "", "<i", "<I", "<i", len_buf_len_str, buf_fmt_str);
...
int
lind_recv_rpc (int sockfd, size_t len, int flags, void *buf)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
  ...
  else
    {
      ...
      if (ptr != NULL)
	{
	  memcpy (ptr, ((&(reply_buffer->contents)) + 0), len);
	}
    }
  free (reply_buffer);
  return return_code;
...
int
lind_connect_rpc (int sockfd, socklen_t addrlen,
		  const struct sockaddr *src_addr)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
...
// check if src_addr is NULL
  ...
  request.format =
    combine (5, "", "<i", "<i", addrlen_src_addr_len_str, src_addr_fmt_str);
  ...
  request.message.body = &args;
  nacl_rpc_syscall_proxy (&request, reply_buffer, 1, src_addr_ptr,
			  src_addr_data_size);
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
    {
...
int
lind_listen_rpc (int sockfd, int backlog)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
int
lind_sendto_rpc (int sockfd, size_t len, int flags, socklen_t addrlen,
		 const struct sockaddr_in *dest_addr, const void *buf)
{
  return 0;
}
...
int
lind_accept_rpc (int sockfd, socklen_t addrlen)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
int
lind_getpeername_rpc (int sockfd, socklen_t addrlen_in, __SOCKADDR_ARG addr,
...
int
lind_setsockopt_rpc (int sockfd, int level, int optname, socklen_t optlen,
		     const void *optval)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
...
// check if optval is NULL
  ...
  request.format =
    combine (7, "", "<i", "<i", "<i", "<i", optlen_optval_len_str,
	     optval_fmt_str);
  ...
  request.message.body = &args;
  nacl_rpc_syscall_proxy (&request, reply_buffer, 1, optval_ptr,
			  optval_data_size);
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
    {
...
int
lind_getsockopt_rpc (int sockfd, int level, int optname, socklen_t optlen,
		     void *optval)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
...
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
  ...
  else
    {
      ...
      if (ptr != NULL)
	{
	  memcpy (ptr, ((&(reply_buffer->contents)) + 0), optlen);
	}
    }
  free (reply_buffer);
  return return_code;
...
int
lind_shutdown_rpc (int sockfd, int how)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
int
lind_select_rpc (int nfds, fd_set * readfds, fd_set * writefds,
		 fd_set * exceptfds, struct timeval *timeout,
		 struct select_results *result)
{
  lind_request request;
...
// check if timeout is NULL
  ...
  request.format =
    combine (10, "", "<i", sizeoffdset_readfds_len_str, readfds_fmt_str,
	     sizeoffdset_writefds_len_str, writefds_fmt_str,
	     sizeoffdset_exceptfds_len_str, exceptfds_fmt_str,
	     sizeofstructtimeval_timeout_len_str, timeout_fmt_str);
  ...
  request.message.body = &args;
  nacl_rpc_syscall_proxy (&request, reply_buffer, 4, readfds_ptr,
			  readfds_data_size, writefds_ptr, writefds_data_size,
			  exceptfds_ptr, exceptfds_data_size, timeout_ptr,
			  timeout_data_size);
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
    {
...
int
lind_getifaddrs_rpc (int ifaddrs_buf_siz, void *ifaddrs)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
  ...
  else
    {
      ...
      if (ptr != NULL)
	{
	  memcpy (ptr, ((&(reply_buffer->contents)) + 0), ifaddrs_buf_siz);
	}
    }
  free (reply_buffer);
  return return_code;
...
int
lind_recvfrom_rpc (int sockfd, size_t len, int flags, socklen_t addrlen,
		   socklen_t * addrlen_out, void *buf,
		   struct sockaddr *src_addr)
{
  lind_request request;
  ...
  int return_code = -1;
  size_t reply_size =
    sizeof (socklen_t) + len + sizeof (struct sockaddr) + sizeof (lind_reply);
  lind_reply *reply_buffer = malloc (reply_size);
  assert (reply_buffer != NULL);
/* These memsets are mainly for saftey.*/
  memset (reply_buffer, 0, reply_size);
...
int
lind_poll_rpc (int nfds, int timeout, struct pollfd *fds_in,
	       struct pollfd *fds_out)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
...
// check if fds_in is NULL
  ...
  request.format =
    combine (5, "", "<i", "<i", sizeofstructpollfdnfds_fds_in_len_str,
	     fds_in_fmt_str);
  ...
  request.message.body = &args;
  nacl_rpc_syscall_proxy (&request, reply_buffer, 1, fds_in_ptr,
			  fds_in_data_size);
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
    {
...
int
lind_socketpair_rpc (int domain, int type, int protocol, int *fds)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
...
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
  ...
  else
    {
      ...
      if (ptr != NULL)
	{
	  memcpy (ptr, ((&(reply_buffer->contents)) + 0), sizeof (int) * 2);
	}
    }
  free (reply_buffer);
  return return_code;
...
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
  ...
  else
    {
      ...
      if (ptr != NULL)
	{
	  memcpy (ptr, ((&(reply_buffer->contents)) + 0), sizeof (uid_t));
	}
    }
  free (reply_buffer);
  return return_code;
...
/* on error return negative so we can set ERRNO. */
  if (reply_buffer->is_error)
  ...
  else
    {
      ...
      if (ptr != NULL)
	{
	  memcpy (ptr, ((&(reply_buffer->contents)) + 0), sizeof (gid_t));
	}
    }
  free (reply_buffer);
  return return_code;
...
int
lind_flock_rpc (int fd, int operation)
{
  lind_request request;
  memset (&request, 0, sizeof (request));
  int return_code = -1;
